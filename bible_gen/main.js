  ...
  .on('data', (row) => {
    ...
    if (current_book === row[Object.keys(row)[0]].match(/(.+?)[a-zA-Z]+/)[0]){
        ...
        if (!isNaN(Number(current_book.charAt(0)))) {
            obj_name = current_book.match(/[a-zA-Z]+/)[0] + "_" + current_book.charAt(0);
        }else {
        ...
        fs.writeFile(`../src/pages/${current_book}.js`, js_template, function(err) {
            if(err) {
                return console.log(err);
            }
        }); 
        fs.writeFile(`../static/bible_txt/${current_book}.txt`, current_string, function(err) {
            if(err) {
                return console.log(err);
            }
        });
